...
void OrbSpl_Assignments(TheInput *TheINP, FILE *f)
{
  ...
  if ((TheINP->Vect2 = (int*)malloc(TheDim*sizeof(int))) == 0)
    exit (EXIT_FAILURE);
  if ((TheINP->ListInvers = (int*)malloc(TheModulo*sizeof(int))) == 0)
    exit (EXIT_FAILURE);
  for (i=1; i<TheModulo; i++)
    {
      iInv=0;
      for (j=1; j<TheModulo; j++)
	{
	  eProd=i*j%TheModulo;
	  if (eProd == 1)
	    iInv=j;
	}
      TheINP->ListInvers[i]=iInv;
    }
}
...
void ConvertLongLong2Vector(TheInput *TheINP, long long *Uin, int **Uout)
{
  ...
  (*Uout)[idx]=1;
  for (iPos=idx+1; iPos<TheINP->TheDim; iPos++)
    (*Uout)[iPos]=0;
  ThePow=1;
  for (iPos=0; iPos<idx; iPos++)
    {
...
{
  long long ThePow, TheLong, TheShift, TheReply;
  int iPos, idxFound, eVal, eValCrit, eValMult, idx, eValInv;
  int eProd;
  idxFound=-1;
  for (iPos=0; iPos<TheINP->TheDim; iPos++)
    {
      eVal=(*Uin)[iPos];
      if (eVal > 0)
	idxFound=iPos;
    }
  eValCrit=(*Uin)[idxFound];
  TheShift=0;
  ThePow=1;
  for (idx=0; idx< idxFound; idx++)
    {
      TheShift=TheShift + ThePow;
      ThePow=ThePow*TheINP->TheModulo;
    }
  eValInv=TheINP->ListInvers[eValCrit];
  ThePow=1;
  TheLong=0;
  for (iPos=0; iPos< idxFound; iPos++)
    {
      eProd=eValInv*(*Uin)[iPos];
      eValMult=eProd % TheINP->TheModulo;
      TheLong=TheLong+eValMult*ThePow;
      ThePow=ThePow*TheINP->TheModulo;
    }
  TheReply=TheShift+TheLong;
  *Uout=TheReply;
}
...
void EFCR_Vector2Creation(TheInput *TheINP, TwoVector *EFRC)
{
  ...
  for (i=0; i<TheINP->TheDim; i++)
    {
      TheNBtotal=TheNBtotal+ThePow;
      ThePow=ThePow*TheINP->TheModulo;
    }
  fprintf(stderr, "TheNBtotal=%lld\n", TheNBtotal);
  h=31;
  TheMod=TheNBtotal % h;
  NBneeded=1 + (TheNBtotal - TheMod)/h;
  fprintf(stderr, "NBneeded=%lld\n", NBneeded);
  EFRC->NBtotal=TheNBtotal;
  TheINP->NBtotal=TheNBtotal;
